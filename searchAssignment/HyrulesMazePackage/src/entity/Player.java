...
public class Player extends MapObject {
	...
	private ArrayList<Action> currentPlan;
	...
	public Player(TileMap tm, ISolver chooser) {
		super(tm);
		this.solver = chooser;
	...
	public void move(){
			...
			Action action = chooseNextMove(tileMap.player_x, tileMap.player_y, tileMap.map);
	...
	public Action chooseNextMove(int x, int y, int[][] map) {
		...
		if(currentPlan == null){
			...
			currentPlan = solver.solve(x, y, gx, gy, map);
		}
		
		if(currentPlan == null) {
			System.err.println("No search implemented! Plan is null!!");
			return Action.NONE;
		}
		...
		Action next =  currentPlan.remove(0);
		return next;
	}
